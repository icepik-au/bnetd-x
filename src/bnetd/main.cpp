...
#ifdef HAVE_GETPID
    eventlog(eventlog_level_info,__FUNCTION__,BNETD_SOFTWARE" version "BNETD_VERSION" process %u",(unsigned int)getpid());
#else
    eventlog(eventlog_level_info,__FUNCTION__,BNETD_SOFTWARE" version "BNETD_VERSION);
#endif
...
try {
	...
	if (!(hexstrm = std::fopen(cmdline_get_hexfile(),"w")))
	    ...
	    std::fprintf(hexstrm,"# dump generated by "BNETD_SOFTWARE" version "BNETD_VERSION"\n");
    }
